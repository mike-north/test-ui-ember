/* globals require */

import { visit, currentURL, findAll } from '@ember/test-helpers';
import { MODE } from 'dummy/tests/utils';

const req = require;

if (MODE === 'qunit') {
  const { setupApplicationTest } = req('ember-qunit');
  const { module, test } = req('qunit');

  module('Acceptance | blue', function (hooks) {
    setupApplicationTest(hooks);
    test('visiting /blue', async function(assert) {
      await visit('/blue');
      assert.equal(currentURL(), '/blue');
    });
  });
} else {
  const { expect } = req('chai');
  const { describe, it } = req('mocha');
  const { setupApplicationTest } = req('ember-mocha');

  describe('Acceptance | blue', function() {
    setupApplicationTest();

    it('can visit /blue', async function() {
      await visit('/blue');
      /** @type {HTMLHeadingElement[]} */
      let H1s = /** @type {any[]} */(findAll('h1'));
      expect(H1s.length).to.equal(1);
      expect(H1s[0].innerText).to.equal('Blue');
      expect(currentURL()).to.equal('/blue');
    });
  });
}
